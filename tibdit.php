...
{ 
  ...
  if (!class_exists("tibditWidget")) 
  {
    ...
    class tibditWidget extends WP_Widget 
    {
      ...
      { 
        ...
        {
        	tiblog("form() has instance $instance");   
        } 
        else 
        {
        ...
        foreach ($setting as $key => $label) 
        {
        	...
        	(
    	   	...
    	   	echo "<input type=text class='bd widefat' id='$item[fid]' name='$item[fname]' value='$item[value]'></p>";
				}
			}
      ...
      function update($new_instance, $old_instance) 
      {
        ...
        if ($new_instance['title'])
        {
          ...
          $instance['title'] = preg_replace_callback("/([Tt][iI][bB])([^ACE-Zace-z][\w]*|$)/", 
            function ($matches) { return strtolower($matches[0]); }, $new_instance['title']);
        }
        ...
        if ($new_instance['intro'])
        {
          ...
          $instance['intro'] = preg_replace_callback("/([Tt][iI][bB])([^ACE-Zace-z][\w]*|$)/", 
            function ($matches) { return strtolower($matches[0]); }, $new_instance['intro']);
        }
      ...
      function widget($args, $instance) // non shortcode widget output
      {
        ...
        $plugurl = plugin_dir_url( __FILE__ );
      	...
      	echo $args['before_widget']."<div class='bd widget'>";
        ...
        {
          tiblog( "tib_post_func() $subref NULL content [[$content]]". var_export($content, true));
          $html = tib_button( $instance['payaddr'], $instance['subref'], $count, null);
        }
        elseif (isset($_COOKIE["tibbed_$instance[subref]"]))
        {
          tiblog( "tib_post_func() $subref TIBBED content [[$content]]");
          $html.=$content;
          $html.=tib_button( $instance['payaddr'], $instance['subref'], $count, null);
        }
        else
        {
          tiblog( "tib_post_func() $subref HIDDEN content "); // . var_export($content, true));
  ...
  function tib_button( $payaddr, $subref, $count, $appearance)
  {
    ...
    {
      $html.="<div class='bd button live' onclick=\"tib('$payaddr','$subref')\"> \n";
    ...
    if(! isset($_COOKIE["tibbed_$subref"]))  // tooltip
    {
      ...
      {
        $html.="<p class=detail><img src='$plugurl/testmode-icon-24px.png' style='height: 1.5em; float:right; padding: 0.2em;'>";
        $html.="testmode tibs are free and carry no value</p>";
      }
      $html.="</div>";
    }
